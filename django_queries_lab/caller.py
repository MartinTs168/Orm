...
def find_authors_nationalities():
    ...
    for author in authors:
        result += f"{str(author)} is {author.nationality}\n"
    return result
...
def filter_authors_by_nationalities(nationality):
    ...
    for author in authors:
        result += (
                author.biography + '\n') if author.biography is not None else f"{author.first_name} {author.last_name}\n"
    return result
...
def change_reviewer_name(curr_name, new_name):
    Review.objects.filter(reviewer_name=curr_name).update(reviewer_name=new_name)
